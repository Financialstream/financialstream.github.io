  ...
  const i18n = {
    en: {
      ...
      cta: {
        ...
        ctaText: 'Book a free consultation or send a request â€” weâ€™ll reply shortly with the next step.',
      ...
      hero: {
        ...
        body: 'We help U.S. small businesses and individuals with accounting, tax, and reporting: QuickBooks Online bookkeeping, sales tax and state reporting, payroll, and business/personal tax return preparation. If you are starting from scratch, catching up past periods, or preparing for filing without stress, we build a clear step-by-step process and handle the routine work. We work remotely across the United States in English and Russian.',
      ...
      reviews: {
        ...
        subtitle: 'A few short notes from clients we have supported across the U.S.',
      ...
      faq: {
        ...
        q1: { q: 'What do you need from me to get started?', a: 'Weâ€™ll ask a few questions and request access to your bookkeeping platform, plus the key documents we need to begin.' },
        q2: { q: 'Do I need to come to your office?', a: 'No. We work fully online and by phone, so you do not need to visit an office. We support clients across the U.S.' },
        ...
        q4: { q: 'Can you clean up months of backlog?', a: 'Yes. We can catch up prior months, fix categorization, and move you into a stable monthly process.' },
        ...
        q8: { q: 'How fast do you respond?', a: 'Typically within 1 business day. Complex cases may take longer â€” weâ€™ll keep you updated along the way.' },
        q9: { q: 'How much does it cost?', a: 'Pricing depends on volume and complexity. Weâ€™ll provide a clear quote after a short call.' },
      },
      contact: {
        title: 'Contact',
        subtitle: 'Tell us your goal and timeline â€” we will reply with a clear next step.',
        formTitle: 'Book a free consultation or ask a question',
        formSub: 'Weâ€™ll review your situation, ask a few quick questions, and suggest the next best step.',
        phone: 'Phone',
        email: 'Email',
        website: 'Website',
      
        location: 'Location',
        locationValue: 'Serving clients remotely across the U.S. Based in Federal Way, Washington.',},
      form: {
      ...
      policyPrivacy: {
        ...
        sms1: 'If you consent to SMS, we may send service-related text messages only in response to your request, such as missed-call follow-up, consultation scheduling, or status updates. We do not send marketing campaigns. Message frequency depends on the situation. Carrier rates may apply.',
      ...
      policyTerms: {
        ...
        sms1: 'If you opt in to SMS, you agree to receive service-related text messages only for your request, including missed-call follow-up, consultation scheduling, and status updates. We do not send marketing campaigns. Message frequency depends on the situation. Carrier rates may apply.',
...
// FS_mobile_contact_order_v1: reorder homepage contact section on small screens.
(function(){
  try {
    var MOBILE_CONTACT_QUERY = '(max-width: 760px)';

    function rememberNodePosition(node) {
      if (!node || node.__fsOriginalParent) return;
      node.__fsOriginalParent = node.parentNode;
      node.__fsOriginalNextSibling = node.nextSibling;
    }

    function restoreNodePosition(node) {
      if (!node || !node.__fsOriginalParent) return;
      var parent = node.__fsOriginalParent;
      var next = node.__fsOriginalNextSibling;
    ...
    function syncMobileContactOrder() {
      var container = document.querySelector('#contact .contact');
      if (!container) return;

      var left = container.querySelector('.contact__left');
      var right = container.querySelector('.contact__right');
      var updates = left ? left.querySelector('.updates-teaser') : null;
      if (!left || !right || !updates) return;

      rememberNodePosition(right);
      rememberNodePosition(updates);

      var isMobile = window.matchMedia ? window.matchMedia(MOBILE_CONTACT_QUERY).matches : window.innerWidth <= 760;

      if (isMobile) {
        if (updates.parentNode !== container || updates !== container.firstElementChild) {
          container.insertBefore(updates, container.firstChild);
        }
        if (right.previousElementSibling !== updates) {
          container.insertBefore(right, left);
        }
      } else {
        restoreNodePosition(updates);
        restoreNodePosition(right);
      }
    }

    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', syncMobileContactOrder);
    } else {
      syncMobileContactOrder();
    }

    window.addEventListener('resize', syncMobileContactOrder, { passive: true });
    window.addEventListener('pageshow', syncMobileContactOrder);
  } catch (_) {
    // no-op
  }
})();
